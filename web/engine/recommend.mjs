...
export function recipeMetrics(recipe, rule = {}) {
  ...
  const currentProfit = useHistoricalCost
    ...
    : websiteProfit;
  const conservativeProfit = !useHistoricalCost && native?.conservativeProfit != null && Number.isFinite(Number(native.conservativeProfit))
    ? Number(native.conservativeProfit) : currentProfit;
  const highProfit = !useHistoricalCost && native?.highProfit != null && Number.isFinite(Number(native.highProfit))
