...
export function saleScenariosForCandidate(candidate, historyAnalysis) {
  ...
  if (native) {
    const conservativeProfit = native.conservativeProfit != null && Number.isFinite(Number(native.conservativeProfit)) ? Number(native.conservativeProfit) : Number(candidate.websiteProfit);
    const highProfit = native.highProfit != null && Number.isFinite(Number(native.highProfit)) ? Number(native.highProfit) : conservativeProfit;
    return {
      ...candidate,
