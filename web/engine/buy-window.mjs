...
export function buildWeeklyBuyAdvice({
  ...
  const basketP15 = percentile(allBasketPrices, pricePercentiles.days14);
  const basketP05 = percentile(allBasketPrices, pricePercentiles.days30);
  const basketP30 = percentile(allBasketPrices, pricePercentiles.days7);
  ...
  const materials = materialProfiles.map(profile => materialAdvice(profile, pricePercentiles));
...
function addRecipeDemand(materials, item, candidate, watchOnly) {
  const runs7 = candidate.runsPerWeek ?? plannedWeeklyRuns(item.place, Number(candidate.hours));
  if (!(runs7 > 0)) return;
  for (const ingredient of candidate.recipe?.materials ?? []) {
    ...
    if (acquisition?.mode === 'exchange' && Number(acquisition.outputCount) > 0) {
      for (const source of acquisition.sources ?? []) {
        ...
        addDemand(materials, {
          name: source.name,
          currentPrice: source.currentPrice == null ? null : Number(source.currentPrice),
          demand,
          watchOnly,
    ...
    addDemand(materials, {
      name,
      currentPrice: ingredient.current_price == null ? null : Number(ingredient.current_price),
      demand,
      watchOnly,
      recipeName: candidate.name,
...
function buildRecipeSignature(recommendations) {
  const recipes = recommendations.map(item => {
    ...
    const materials = (selected.recipe?.materials ?? [])
      ...
      .sort().join(',');
    return `${item.place}:${selected.recipe?.id ?? normalizeName(selected.name)}:${selected.hours}:${selected.runsPerWeek ?? plannedWeeklyRuns(item.place, selected.hours)}:${materials}`;
  }).sort().join('|');
...
function materialAdvice(profile, pricePercentiles) {
  ...
  const p05 = sufficientlyPriced ? percentile(values, pricePercentiles.days30) : null;
  const p15 = sufficientlyPriced ? percentile(values, pricePercentiles.days14) : null;
  const p30 = sufficientlyPriced ? percentile(values, pricePercentiles.days7) : null;
  const medianPrice = sufficientlyPriced ? percentile(values, 0.50) : null;
  // The displayed ceiling must include the same minimum discount as the signal.
  // Prices are whole Haff coins; strict < median * discount becomes ceil(...) - 1.
  const ceiling = (quantile, discount) => quantile === null ? null : Math.max(0, Math.min(Math.floor(quantile), Math.ceil(medianPrice * discount) - 1));
  const thresholds = { days7: ceiling(p30, 0.99), days14: ceiling(p15, 0.98), days30: ceiling(p05, 0.97) };
  const targetPrice = thresholds.days7;
  let tierDays = 0;
  if (!ignored && Number.isFinite(material.currentPrice) && medianPrice > 0) {
    if (material.currentPrice <= thresholds.days30) tierDays = 30;
    else if (material.currentPrice <= thresholds.days14) tierDays = 14;
    else if (material.currentPrice <= thresholds.days7) tierDays = 7;
  }
  ...
  return {
    ...
    tierDays,
    tierThresholds: sufficientlyPriced ? thresholds : null,
